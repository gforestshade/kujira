...
`age = a.getAge()`では「Hito型の変数`a`の」関数である
`getAge()`を呼び出しています。
`getAge()`はreturn文で`18`を返しますから、
`age`には`18`が代入されます。
...
なので、`getAge()`を処理するにあたって自分の持ち主たるインスタンス`a`の情報を利用できるように、
`self`に`a`が代入された状態で`getAge()`が呼び出されるわけです。

クラス定義ブロックの中にある関数のことを特に「メソッド」とよびます。
普通にメソッドを定義するとインスタンスの所有物としてのメソッドになり、
そのようなメソッドを「インスタンスメソッド」と呼びます。
`インスタンス.インスタンスメソッド名(追加の引数)`とすることでインスタンスメソッドを呼び出すことができます。
...
レシーバは自動的に設定されますので、それ以外の引数を指定しましょう。
...
同じクラス(型)から異なる複数のインスタンス(値)が作れるので、
